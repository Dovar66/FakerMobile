...
import com.dovar.fakermobile.fms.Phone;
import com.dovar.fakermobile.fms.Resolution;
import com.dovar.fakermobile.fms.XBuild;
...
public class XposedUtil implements IXposedHookLoadPackage {
    ...
    @Override//应用启动需要从zygote孵化器中fork进程，此时会回调handleLoadPackage()
    public void handleLoadPackage(XC_LoadPackage.LoadPackageParam mLoadPackageParam) throws Throwable {
        ...
        if (mLoadPackageParam.packageName.equals(MainActivity.targetPackage)) {
            //获取最新数据
            DataUtil.init();
...
/*        new XC_MethodHook() {
    ...
    private void initPackageManager(final XC_LoadPackage.LoadPackageParam lpparam) {
        try {
            XposedHelpers.findAndHookMethod(ActivityManager.class.getName(), lpparam.classLoader, "getRecentTasks", Integer.TYPE.getName(), Integer.TYPE.getName(), new XC_MethodHook() {
                @Override
                protected void afterHookedMethod(MethodHookParam param) throws Throwable {
...
//                    param.setResult(null);
    ...
    private int DURATION_INTERVAL = 10 * 1000;//限制模拟摇一摇最小时间间隔，即连续两次摇一摇的最小时间间隔
    private long lastUpdateTime; // 上次检测时间
    /**
     * hook传感器
     * 模拟摇一摇或用于修改步数
     */
    private void hookSensor(XC_LoadPackage.LoadPackageParam loadPackageParam) {
